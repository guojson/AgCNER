...
def simple_run():
    ...
    data_train = np.load(config.train_dir, allow_pickle=True)
    word_train = data_train["words"]
...
def run(word_train, label_train, word_dev, label_dev, vocab, device, kf_index=0):
    ...
    train_loader = DataLoader(train_dataset, batch_size=config.batch_size,
                              shuffle=True, collate_fn=train_dataset.collate_fn, num_workers=8)

    dev_loader = DataLoader(dev_dataset, batch_size=config.batch_size,
                            shuffle=True, collate_fn=dev_dataset.collate_fn, num_workers=8)

    # model
    model = BiLSTM_CRF(config=config, embedding_size=config.embedding_size,
