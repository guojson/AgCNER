...
import torch
from V3.modules import Transformer_Encoder
import collections
...
class Lattice_Transformer_SeqLabel(nn.Module):
    def __init__(self,lattice_embed, bigram_embed, hidden_size, label_size,
        ...
        if self.use_rel_pos:
            ...
            self.pe = nn.Embedding(2*max_seq_len + 1, self.num_heads)
            if self.four_pos_shared:
                ...
                self.pe_ss = nn.Embedding(2*max_seq_len + 1, self.num_heads)
                self.pe_se = nn.Embedding(2*max_seq_len + 1, self.num_heads)
                self.pe_es = nn.Embedding(2*max_seq_len + 1, self.num_heads)
                self.pe_ee = nn.Embedding(2*max_seq_len + 1, self.num_heads)
        else:
            self.pe = None
            self.pe_ss = None
            self.pe_se = None
