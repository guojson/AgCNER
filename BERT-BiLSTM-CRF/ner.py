...
from models import BERT_BiLSTM_CRF, BERT_IDCNN_CRF
import conlleval
...
def evaluate(args, data, model, id2label, all_ori_tokens, logger, mode='eval'):
    model.eval()
    sampler = SequentialSampler(data)
    dataloader = DataLoader(data, sampler=sampler, batch_size=args.train_batch_size, num_workers=6)
    ...
    eval_list = []
    result_file = 'eval_results.txt'
    if mode == 'test':
        result_file = 'predict_results.txt'

    with open(os.path.join(args.output_dir, result_file), "w", encoding="utf-8") as f:
    ...
    conlleval.report(counts, logger=logger)
...
def main():
    ...
    ## Required parameters
    parser.add_argument("--train_file", default="data/agner/train.txt", type=str)
    parser.add_argument("--eval_file", default="data/agner/dev.txt", type=str)
    parser.add_argument("--test_file", default="data/agner/test.txt", type=str)
    parser.add_argument("--model_name_or_path", default="bert-base-chinese", type=str)
    parser.add_argument("--output_dir", default="agcner/bert_crf", type=str)
    ## other parameters
    ...
    if args.do_train:
        ...
        # model = BERT_IDCNN_CRF.from_pretrained(args.model_name_or_path, config=config,
        #                                         need_birnn=args.need_birnn, rnn_dim=args.rnn_dim)
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                        ...
                        logger.info("Train/loss" + str(tr_loss_avg) + str(global_step))
                        # writer.add_scalar("Train/loss", tr_loss_avg, global_step)
                        logging_loss = tr_loss

            if args.do_eval:
                all_ori_tokens_eval = [f.ori_tokens for f in eval_features]
                overall, by_type = evaluate(args, eval_data, model, id2label, all_ori_tokens_eval, logger)
    ...
    if args.do_test:
        ...
        # model = BERT_IDCNN_CRF.from_pretrained(args.output_dir, need_birnn=args.need_birnn, rnn_dim=args.rnn_dim)
        model.to(device)
        ...
        all_ori_tokens_test = [f.ori_tokens for f in test_features]
        overall, by_type = evaluate(args, test_data, model, id2label, all_ori_tokens_test, logger, mode='test')

        # add eval result to tensorboard
        # f1_score = overall.fscore
        # writer.add_scalar("Test/precision", overall.prec, ep)
        # writer.add_scalar("Test/recall", overall.rec, ep)
        # writer.add_scalar("Test/f1_score", overall.fscore, ep)
        ...
        #
        #
        # with open(os.path.join(args.output_dir, "predict_results.txt"), "w", encoding="utf-8") as f:
...
if __name__ == "__main__":
    # args = torch.load('agcner/bert_bilstm-crf/1/training_args.bin')
    #
    # print(args)
    main()
    # pass
